...
SET_COST_FUNCTION (minimize_weighted_activation_time_with_angle_restriction)
{
    FILE *log_file = the_network->log_file;
    ...
    bool ret;
    double w;
    ret = get_parameter_value_from_map(config->params,"w",&w);
    double G;
    ret = get_parameter_value_from_map(config->params,"G",&G);
    double Cf;
    ret = get_parameter_value_from_map(config->params,"Cf",&Cf);
    ...
    for (uint32_t i = 0; i < feasible_segments.size(); i++)
    {
        ...
        if (using_local_optimization)
        {
            ...
            // 2) Calculates total activation time
            double eval_2 = calc_total_activation_time(the_network,G,Cf,tau_f);

            // 3) Weighted sum
            double eval = (w)*eval_1 + (1-w)*eval_2;
            ...
            // Level 
            //bool has_minimum_level = has_level(inew,num_terminals);
