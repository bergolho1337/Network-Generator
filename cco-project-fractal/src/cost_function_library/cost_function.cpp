...
SET_COST_FUNCTION (minimize_tree_activation_time_with_angle_restriction)
{
    ...
    get_parameter_value_from_map(config->params,"degrees_min",&min_degrees_limit);
    double max_degrees_limit;
    get_parameter_value_from_map(config->params,"degrees_max",&max_degrees_limit);
    ...
    for (uint32_t i = 0; i < feasible_segments.size(); i++)
    {
        ...
        if (at < minimum_at && \
            !has_deviation(the_network->segment_list,inew,at,deviation_limit,c,cm,rc,rm) && \
            degrees >= min_degrees_limit && degrees <= max_degrees_limit)
        {
            best = iconn;
            minimum_at = at;
        }
