def write_monoalg_main_section (file):
    file.write("[main]\n")
...
def write_monoalg_save_result_section (file,seed,use_cluster=False):
    file.write("[save_result]\n")
    file.write("print_rate = 25\n")
    file.write("output_dir = ./outputs/elizabeth_dla_seed:%u\n" % (seed))
...
def write_monoalg_purkinje_section (file,seed,use_cluster=False):
    file.write("[purkinje]\n")
    ...
    file.write("network_file=networks/03_Lucas/01_SRN/04_DLA/seed:%u.vtk\n" % (seed))
...
def write_monoalg_config_file (seed):
    filename = "monoalg3d/dla_seed-%u.ini" % (seed)
    file = open(filename,"w")

    write_monoalg_main_section(file)
    write_monoalg_update_monodomain(file)
    write_monoalg_save_result_section(file,seed)
    ...
    write_monoalg_purkinje_section(file,seed)
