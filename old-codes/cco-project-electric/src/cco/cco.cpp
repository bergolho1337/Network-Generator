...
struct cco_network* new_cco_network (struct user_options *options)
{
    ...
    result->I_in = options->I_in;
    result->V_in = options->V_in;
...
{
    // Set the current and potential of this segment
    iroot->value->I = I_in;
    iroot->value->delta_v = delta_v;
    ...
    double R = RHO * length / M_PI;
    iroot->value->resistance = R;

    // Calculate the radius using Ohm's law
    iroot->value->radius = pow(R * I_in / delta_v , 0.5);
                 ...
                 const double I_in, const double delta_v, const int num_terminals)
{
    ...
    if (iconn->value->left == NULL && iconn->value->right == NULL)
    {
        ...
        calc_potential_drop_term(iconn,I_out);
    }
    else
    {
    ...
    if (ipar != NULL)
    {
        ...
        rescale_until_root(ipar,ipar_left,ipar_right,I_in,delta_v,num_terminals);
    }
    // We are already at the root, so recalculate the radius using (2.19)
    else
    {
        ibiff->value->radius = pow(ibiff->value->resistance * I_in / delta_v , 0.5);
    }
}
                        ...
                        const double I_in, const double delta_v, const int num_terminals)
{
    ...
    if (ipar_left != NULL && ipar_right != NULL)
    {
        ...
        double radius_ratio = calc_radius_ratio(ipar_right,ipar_left,I_out);
                                ...
                                const uint32_t index, const double new_pos[])
{
    ...
    struct segment *ibiff = new_segment(iconn_node->value->src,M,\
                            NULL,NULL,iconn_node->value->parent,I_in,V_in);
    ...
    rescale_tree(ibiff_node,iconn_node,inew_node,I_in,delta_v,the_network->num_terminals);
                        ...
                        struct segment_node *iconn)
{
    ...
    if (ipar != NULL)
    {
        ...
        rescale_until_root(ipar,ipar_left,ipar_right,\
                        I_in,delta_v,the_network->num_terminals);
    }
    // Recalculate the root radius when we reach this segment using (2.19)
    else
    {
        iconn->value->radius = pow(iconn->value->resistance * I_in / delta_v , 0.5);
    }
...
void calc_relative_resistance_term (struct segment_node *iterm)
{
    ...
    iterm->value->resistance = RHO * length / M_PI;
}

void calc_potential_drop_term (struct segment_node *iterm, const double I_out)
{
    iterm->value->delta_v = iterm->value->resistance * I_out;
}
...
void calc_potential_drop_subtree (struct segment_node *iconn, const double I_out)
{
    ...
    iconn->value->delta_v = iconn->value->resistance * I_iconn;
}
