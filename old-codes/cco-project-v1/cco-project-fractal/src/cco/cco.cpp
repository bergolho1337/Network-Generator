...
struct cco_network* new_cco_network (struct user_options *options)
{
    ...
    result->start_radius = options->start_radius;
...
void grow_tree_using_cloud_points (struct cco_network *the_network, struct user_options *options)
{
    ...
    double start_radius = the_network->start_radius;
    double Q_perf = the_network->Q_perf;
    double p_perf = the_network->p_perf;
    double p_term = the_network->p_term;
    double r_perf = the_network->r_perf;
...
void rescale_root (struct segment_node *iroot, const double Q_perf, const double delta_p, const double start_radius)
{
    ...
    // [FRACTAL]
    // Calculate the root radius using the user input parameter
                 ...
                 const double Q_perf, const double delta_p, const double start_radius, const int num_terminals)
{
    ...
    if (ipar != NULL)
    {
        ...
        rescale_until_root(ipar,ipar_left,ipar_right,Q_perf,delta_p,start_radius,num_terminals);
    }
    ...
    else
    {
        ...
        // [FRACTAL]
        // We are already at the root, so jus copy the start_radius value
    ...
    if (ipar_left != NULL && ipar_right != NULL)
    {
        ...
        if (ipar->value->parent != NULL) 
            rescale_until_root(ipar->value->parent,\
                               ...
                               Q_perf,delta_p,start_radius,num_terminals);
...
struct segment_node* build_segment (struct cco_network *the_network, const uint32_t index, const double new_pos[])
{
    ...
    rescale_tree(ibiff_node,iconn_node,inew_node,Q_perf,delta_p,start_radius,the_network->num_terminals);
                        ...
                        struct segment_node *iconn)
{
    ...
    if (ipar != NULL)
    {
        ...
        rescale_until_root(ipar,ipar_left,ipar_right,\
                        Q_perf,delta_p,start_radius,the_network->num_terminals);
    }
    // Recalculate the root radius when we reach this segment using (2.19)
    else
    {
...
void make_root_using_cloud_points (struct cco_network *the_network, std::vector<struct point> cloud_points)
{
    ...
    rescale_root(iroot_node,Q_perf,delta_p,start_radius);
