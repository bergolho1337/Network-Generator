...
void Graph::insert_node_graph (const uint32_t index, const double pos[])
{

    Node *node = new Node(index,pos);
...
{
    // ERROR! Source and destination are equal !
    if (id_1 == id_2) return;
...
void Graph::breadth_first_search (vector<Node> &mapped_points, const uint32_t root_index)
{
    uint32_t index;
    double pos[3];

    int source_index = root_index;
    Node *source_node = search_node(source_index);
    ...
    while (!q.empty())
    {
        ...
        // Add this point to the mapped points
        index = u->index; 
        pos[0] = u->x;
        pos[1] = u->y;
        pos[2] = u->z;
        Node node(index,pos);
        mapped_points.push_back(node);
...
{
    FILE *file = fopen("outputs/mapped_points.pts","w+");
...
{
    char filename[200];
    sprintf(filename,"outputs/mapped_points_%u.pts",iter);
    FILE *file = fopen(filename,"w+");
