...
public:
	...
	void breadth_first_search (vector<Node> &mapped_points, const uint32_t root_index);
	//void dijkstra (int s);
	// Inline
	...
	void insert_node_graph (const uint32_t index, const double pos[]);
...
public:
	...
	uint32_t face_index;
public:
	Link (const uint32_t point_index, const uint32_t face_index)
	{
